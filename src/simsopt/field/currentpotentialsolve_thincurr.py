...
class CurrentPotentialSolve:
    ...
    def __init__(self, cp, plasma_surface, Bnormal_plasma):
        self.current_potential = cp #current potential class
        self.winding_surface = self.current_potential.winding_surface #access winding surface
        self.ndofs = self.current_potential.num_dofs() #fourier coeff
        ...
        # Calculating B_GI
        cp_no_phi_sv = CurrentPotentialFourier( #single valued current potential
        ...
        Bfield = WindingSurfaceField(cp_no_phi_sv) #obtain B field from the single valued potential
        points = plasma_surface.gamma().reshape(-1, 3)
        Bfield.set_points(points)
        B_GI_vector = Bfield.B() #vector of secular terms
        ...
        self.B_GI = B_GI_winding_surface #These are the secular terms
    ...
    def B_matrix_and_rhs(self):
        ...
        #B field from single valued current potential can be written as a fourier series. 
        #gj are the sines and cosines that make up the series
        B_GI = self.B_GI

        # set up RHS of optimization
        b_rhs = - np.ravel(B_GI + Bnormal_plasma) @ gj #secular terms plus normal field component @ 
        ...
        G = self.current_potential.net_poloidal_current_amperes #G secular term
        I = self.current_potential.net_toroidal_current_amperes #I secular terms
    ...
    def solve_tikhonov(self, lam=0, record_history=True):
        ...
        """
        #This is the money. This is what we need Thincurr to replace
